    ...
    override func render(canvas: Canvas) {
        ...
        if let canvasSize = canvas.canvasSize {
            if (foregroundLayer().gameOver) {
                let rectangle = Rectangle(rect: Rect(topLeft: Point(x: canvasSize.width / 3, y: canvasSize.height / 3),
                                                     size: Size(width: canvasSize.width / 4, height: canvasSize.height / 3)),
                                          fillMode: .fillAndStroke)
                let gameOverText = Text(location: Point(x: rectangle.rect.topLeft.x + rectangle.rect.width / 2, y: rectangle.rect.topLeft.y + rectangle.rect.height / 2), text: "Null")
